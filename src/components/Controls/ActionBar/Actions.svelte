...
<div class="action-buttons space-x-3">
	...
	<input class="test-input absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-56 px-3 py-2 rounded-lg border border-gray-300 shadow-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary transition-all z-10" class:visible={isTest} class:invisible={!isTest} bind:value={testFuncName} on:keydown={handleTest} placeholder="输入测试函数名，回车运行"/>
	...
	<button class="btn btn-round" disabled={$gamePaused} on:click={handleBackTracking} title="BackTracking">
		<svg class="icon-outline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	{#if $possibleNumberSwitch}
		<button class="btn btn-round" disabled={$gamePaused} on:click={possibleNumberSwitch.toggle} title="Show Possible Numbers">
			<svg class="icon-outline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	{:else}
		<button class="btn btn-round" disabled={$gamePaused} on:click={possibleNumberSwitch.toggle} title="Hide Possible Numbers">
			<svg class="icon-outline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
